...
class PPO(nn.Module):
    def __init__(self,state_dim,action_dim):
        ...
        self.mean = nn.Linear(in_features=64,
                                   out_features=self.dim_action)
        nn.init.orthogonal_(self.mean.weight, np.sqrt(0.01))
        self.tanh = nn.Tanh()
        self.value = nn.Linear(in_features=64,
                               out_features=1)
        
        self.std = nn.Parameter(t.zeros(action_dim))
    ...
    def forward(self, x):
        ...
        mean  = self.tanh(self.mean(h))
        pi = Normal(mean, 2 * F.softplus(self.std))
        value = self.value(h).reshape(-1)
        return pi, value
